...
card.className='card';
card.innerHTML=`<img src="${unit.gallery[0]}"><div class="card-content"><span class="${badgeClass}">${unit.category.toUpperCase()}</span><h2>${unit.name}</h2><p>${unit.description}</p><div class="stats"><div>${unit.fps}</div><div>${unit.battery}</div></div></div>`;
